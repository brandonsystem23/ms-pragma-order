...
class DishRegistrationValidatorTest {
    ...
    void shouldFailWhenAuthenticatedUserIsNotOwner() {
        ...
        StepVerifier.create(validator.validate("Pizza Hawaiana", 1L, "token-test"))
                .expectErrorSatisfies(error -> {
                    org.junit.jupiter.api.Assertions.assertInstanceOf(DomainException.class, error);
                    org.junit.jupiter.api.Assertions.assertEquals("No tienes permisos para crear platos", error.getMessage());
                })
                .verify();
    }
    ...
    void shouldFailWhenRestaurantDoesNotExist() {
        ...
        StepVerifier.create(validator.validate("Pizza Hawaiana", 1L, "token-test"))
                .expectErrorSatisfies(error -> {
                    org.junit.jupiter.api.Assertions.assertInstanceOf(DomainException.class, error);
                    org.junit.jupiter.api.Assertions.assertEquals("El restaurante no existe", error.getMessage());
                })
                .verify();
    }
    ...
    void shouldFailWhenUserIsNotOwnerOfRestaurant() {
        ...
        StepVerifier.create(validator.validate("Pizza Hawaiana", 1L, "token-test"))
                .expectErrorSatisfies(error -> {
                    org.junit.jupiter.api.Assertions.assertInstanceOf(DomainException.class, error);
                    org.junit.jupiter.api.Assertions.assertEquals("Usted no es propietario del restaurante", error.getMessage());
                })
                .verify();
    }
    ...
    void shouldFailWhenDishNameAlreadyExists() {
        ...
        StepVerifier.create(validator.validate("Pizza Hawaiana", 1L, "token-test"))
                .expectErrorSatisfies(error -> {
                    org.junit.jupiter.api.Assertions.assertInstanceOf(DomainException.class, error);
                    org.junit.jupiter.api.Assertions.assertEquals("El nombre del palto ya está registrado", error.getMessage());
                })
                .verify();
    }
}
